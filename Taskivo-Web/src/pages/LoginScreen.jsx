...
export function LoginScreen({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!token || !user) { setError("Login failed. Please try again."); return; }
      if (!user.role)       { setError("No role assigned. Contact your administrator."); return; }
      onLogin(token, user);
    } catch (err) {
  ...
  const inputStyle = {
    ...
    border: "1.5px solid #e4e4e4", fontSize: 14, color: "#111",
    boxSizing: "border-box", outline: "none", background: "#fafafa",
    transition: "border 0.15s, background 0.15s",
  };
  const focus = e => { e.target.style.border = "1.5px solid #2c47c5"; e.target.style.background = "#fff"; };
  const blur  = e => { e.target.style.border = "1.5px solid #e4e4e4"; e.target.style.background = "#fafafa"; };
    ...
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#f5f6fa" }}>
      <div style={{ width:"100%", maxWidth:420, background:"#fff", borderRadius:16, padding:"44px 40px", boxShadow:"0 4px 32px rgba(0,0,0,0.08)" }}>
        ...
        <button
          ...
          onMouseEnter={e => { if(!loading) e.target.style.background="#1e35a8"; }}
          onMouseLeave={e => { if(!loading) e.target.style.background="#2c47c5"; }}
        >
          {loading
            ? <><i className="fa-solid fa-spinner fa-spin" style={{ marginRight:8 }}/>Please wait...</>
            : "Sign In"
          }
        </button>
